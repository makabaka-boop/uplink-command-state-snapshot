...
- **控制台读取自洽**：指令状态、各代租约与结算记录在同一快照事务内读取，
  绝不出现“旧领取代数配新代租约”或“待处理且已结算”的矛盾页面。
- **过期边界跨实例一致**：租约签发、领取资格与回执过期判定统一使用数据库时钟，
  实例时钟偏差不参与裁决，任何实例对同一张凭证得出相同结论。
...
- **快照一致性**：状态、各代租约与结算记录在同一个只读 `REPEATABLE READ` 事务内
  读取。查询恰好跨过另一终端的领取提交或回执提交时，页面只会呈现提交前或提交后的
  完整状态，绝不拼接出“旧代数配新租约”“`pending` 配已生成结算”这类矛盾组合；
  失败传播出的 `blocked` 也要么整体可见、要么整体不可见。
...
**时钟模型**：`expires_at` 的签发、领取资格判定（`lease_expires_at <= now()`）与回执
过期判定（`expires_at > now()`）全部在 SQL 内以数据库时钟完成，不读取任何应用实例的
本地时钟。多实例部署时，同一张尚在有效期的凭证不会在一台被判过期，已过期凭证也不会
在另一台被接受——过期边界在各实例上得出相同结论。
...
10. **跨实例过期边界**：主实例与重启实例对同一凭证结论一致——有效凭证互相接受、
    过期凭证互相拒绝、判过期后另一实例可立即重领（数据库时钟为唯一裁决时钟）；
11. **读取一致性**：领取/回执交错压力下高频读取控制台详情页，断言每一页都自洽
    （无跨代租约、无“待处理且已结算”）；
12. **失败一致性**：失败回执后，失败根呈现 `failed` + 对应结算记录，下游呈现
    `blocked` + 阻断来源且无租约无结算，不可领取。
